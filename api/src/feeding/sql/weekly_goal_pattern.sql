...
SELECT 
      ...
      ,QT_SOLIC
      ,TOTAL_SEMANAS
      ,Round(CASE WHEN SEMANA = 1 THEN ((SELECT tscm_calculo_metas('FECHADO',1,'S') FROM dual)*100)/TOTAL_SEMANAS END ,1)percentual_atendido_semana1
      ,Round(CASE WHEN SEMANA = 2 THEN ((SELECT tscm_calculo_metas('FECHADO',2,'S') FROM dual)*100)/TOTAL_SEMANAS END ,1)percentual_atendido_semana2
      ,Round(CASE WHEN SEMANA = 3 THEN ((SELECT tscm_calculo_metas('FECHADO',3,'S') FROM dual)*100)/TOTAL_SEMANAS END ,1)percentual_atendido_semana3
      ,Round(CASE WHEN SEMANA = 4 THEN ((SELECT tscm_calculo_metas('FECHADO',4,'S') FROM dual)*100)/TOTAL_SEMANAS END ,1)percentual_atendido_semana4
      ,Round(CASE WHEN SEMANA = 5 THEN ((SELECT tscm_calculo_metas('FECHADO',5,'S') FROM dual)*100)/TOTAL_SEMANAS END ,1)percentual_atendido_semana5
...
SELECT  TIPO
        ...
        ,Nvl(CASE WHEN SEMANA = 1  THEN (SELECT tscm_calculo_metas('ABERTO',1,'S') teste FROM dual) END,0) aberto_acrescimo1

        ,Nvl(CASE WHEN SEMANA = 2  THEN (SELECT tscm_calculo_metas('ABERTO',2,'S') teste FROM dual) END,0) aberto_acrescimo2
        
        ,Nvl(CASE WHEN SEMANA = 3  THEN (SELECT tscm_calculo_metas('ABERTO',3,'S') teste FROM dual) END,0) aberto_acrescimo3
        
        ,Nvl(CASE WHEN SEMANA = 4  THEN (SELECT tscm_calculo_metas('ABERTO',4,'S') teste FROM dual) END,0)aberto_acrescimo4 
       
        ,Nvl(CASE WHEN SEMANA = 5 THEN  (SELECT tscm_calculo_metas('ABERTO',5,'S') teste FROM dual) END,0) aberto_acrescimo5 
