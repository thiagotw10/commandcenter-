...
FROM(SELECT A.CD_COR_REFERENCIA,
COR.DS_REFERENCIA DS_REFERENCIA,
FILA.DS_FILA FILA,
count(*) as ESPERA_ATENDIMENTO,
--a.cd_cor_referencia,
...
SubStr(CAST(SYSDATE AS TIMESTAMP) - Max(CAST(B.DH_PROCESSO AS TIMESTAMP)), 11,9)  MAIOR_TEMPO_ESPERA
...
AND C.cd_prestador in (161,7347)
...
AND ROWNUM = 1)
group by a.cd_cor_referencia,COR.DS_REFERENCIA, FILA.DS_FILA , DH_PROCESSO
)
